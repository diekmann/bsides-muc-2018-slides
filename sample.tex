...
\begin{tikzpicture}[overlay]
	        ...
	        \node<2> (wat) at (current page.center) {\resizebox{.2\textwidth}{!}{\verb~-i~ \textcolor{red}{\Snowman}}};
	        \node<3> (wat) at (current page.center) {\resizebox{.4\textwidth}{!}{\verb~-i~ \textcolor{red}{\Snowman}}};
	        \node<4> (wat) at (current page.center) {\resizebox{.8\textwidth}{!}{\verb~-i~ \textcolor{red}{\Snowman}}};
	        
	        %\node<5> (snow1) at ($(current page.center) + (0,-2ex)$) {\frame{\includegraphics[width=.80\linewidth]{iplinksnowman.png}}};
	\end{tikzpicture}
\end{frame}
...
\begin{frame}
    ...
    \begin{itemize}
	    ...
	    \item \only<1,2,3>{$\bigstep{\free{\mvar{rs}}}{\undecided}{\allow}$} \only<2->{Firewall ruleset $\free{\mvar{rs}}$ accepts packet $\free{p}$}
	    \begin{itemize}
	    	\item<3-> gamma is all Linux matching features
	    	\item<4-> placeholder
