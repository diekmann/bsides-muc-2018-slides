...
\begin{frame}[fragile]
    ...
    \begin{center}%
	    ...
	    \end{IEEEeqnarray*}}
	    \onslide<21->{In the graph, we can find an arrow from \ $\mdef{src}\ \free{\mvar{p}}$ $\longrightarrow$ $\mdef{dst}\ \free{\mvar{p}}$}
	    
	    \onslide<22->{Warning: The other direction may not hold!}
	    \end{overprint}
    \end{center}
    \medskip
