...
\begin{frame}[fragile]
    ...
    \begin{itemize}
	    ...
	    \begin{itemize}
	    	\item $(\mdef{prot}\ \free{p}, \mdef{sport}\ \free{p}, \mdef{dport}\ \free{p})$: Example $(\mvar{tcp}, 42242, 22)$
	    \end{itemize}
    \end{itemize}
    Shows%
    \begin{center}%
	    ...
	    \exists \highlight<10>{\bound{\mvar{s}_\text{repr}}}\ \highlight<10>{\bound{\mvar{d}_\text{repr}}}\ \bound{\mvar{s}_\text{range}}\ \bound{\mvar{d}_\text{range}}.\ \ (\highlight<10>{\bound{\mvar{s}_\text{repr}}}, \highlight<10>{\bound{\mvar{d}_\text{repr}}}) \in  \mdef{set}\ \free{\mvar{E}} \  \wedge  \\
	    \qquad (\mfun{map\_of}\ \free{\mvar{V}})\ \highlight<10>{\bound{\mvar{s}_\text{repr}}} = \mdef{Some}\ \bound{\mvar{s}_\text{range}} \ \wedge \ (\mdef{src}\ \free{\mvar{p}}) \in \bound{\mvar{s}_\text{range}} \ \wedge \\
	    \qquad (\mfun{map\_of}\ \free{\mvar{V}})\ \highlight<10>{\bound{\mvar{d}_\text{repr}}} = \mdef{Some}\ \bound{\mvar{d}_\text{range}} \ \wedge \ (\mdef{dst}\ \free{\mvar{p}}) \in \bound{\mvar{d}_\text{range}}
	    \end{IEEEeqnarray*}
    \end{center}
    \medskip
	...
	% such meta meta!
	\begin{tikzpicture}[overlay]
	    \node<5,6,7,8>[fill=white, draw=black, line width=3pt, inner sep=10pt] (tikzmeta) at (current page.center) {\begin{minipage}{.9\linewidth}
\footnotesize
\texttt{\textbackslash{}begin\{tikzpicture\}}\\
\texttt{\hspace*{1ex}\textbackslash{}node (\highlight<6>{a}) at (-4,-4) \{\highlight<7>{\$\textbackslash{}\{131.159.21.0 .. 131.159.21.255\textbackslash{}\}\$}\};}\\
\texttt{\hspace*{1ex}\textbackslash{}node (b) at (4,-4) \{\$\textbackslash{}\{131.159.15.240 .. 131.159.15.255\textbackslash{}\}\$\};}\\
\texttt{\hspace*{1ex}\textbackslash{}node (c) at (0,-6) \{\$\textbackslash{}\{127.0.0.0 .. 127.255.255.255\textbackslash{}\}\$\};}\\
$\dots$\\

\texttt{\hspace*{1ex}\textbackslash{}draw (\highlight<8>{a}) to (\highlight<8>{a});}\\
\texttt{\hspace*{1ex}\textbackslash{}draw (\highlight<8>{a}) to (\highlight<8>{b});}\\
\texttt{\hspace*{1ex}\textbackslash{}draw (\highlight<8>{a}) to (\highlight<8>{c});}\\
$\dots$\\
